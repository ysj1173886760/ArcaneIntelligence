...
class MoonshotAIProvider(ChatModelProvider, EmbeddingModelProvider):
    ...
    def _parse_completion_message(
        chat_completion_message: ChatCompletion,
    ) -> AssistantChatMessage:
        return AssistantChatMessage(
            role=chat_completion_message.choices[0].message.role,
            content=chat_completion_message.choices[0].message.content,
        )
    ...
    async def create_chat_completion(
        ...
        chat_completion = await self._client.chat.completions.create(
            model=model_name, messages=raw_messages, temperature=0.3
        )

        logging.debug(
            "chat response: {} prompt token used: {} completion token used: {}".format(
        ...
        return ModelResponse(
            result=MoonshotAIProvider._parse_completion_message(chat_completion),
    ...
    async def create_embedding(
        ...
        embedding_response: CreateEmbeddingResponse = (
            await self._client.embeddings.create(input=text, model=model_name)
        )

        logging.debug(
            "create embedding response: {} prompt token used: {} total token used: {}".format(
        ...
        return EmbeddingModelResponse(
            ...
            completion_tokens_used=embedding_response.usage.total_tokens,
        )
