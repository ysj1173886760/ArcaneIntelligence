...
async def main():
    ...
    response = await ai_provider.create_chat_completion(
        messages=chat_history, model_name="glm-4"
    )
...
async def embedding_test():
    ...
    response = await ai_provider.create_embedding(
        "Represent the question for retrieving supporting documents: 飞机",
...
search_with_keywords_func = CompletionModelFunction(
    ...
    description=("Search the document with the keywords"),
    parameters={
